...
// Movie/TV tracking data interfaces
interface MovieData {
  id: string;
  title: string;
  year: number;
...
class MediaStorage {
  private movies: Map<string, MovieData> = new Map();
  private tvShows: Map<string, TVShowData> = new Map();

  // Movie methods
  addMovie(movie: Omit<MovieData, 'id' | 'dateAdded'>): MovieData {
  ...
  getMovie(id: string): MovieData | undefined {
  ...
  updateMovie(id: string, updates: Partial<MovieData>): MovieData | undefined {
  ...
  // TV Show methods
  addTVShow(show: Omit<TVShowData, 'id' | 'dateAdded'>): TVShowData {
    const id = `tv_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const showData: TVShowData = {
      ...show,
      id,
      dateAdded: new Date().toISOString(),
    };
  ...
  getTVShow(id: string): TVShowData | undefined {
    return this.tvShows.get(id);
  }

  getAllTVShows(): TVShowData[] {
    return Array.from(this.tvShows.values());
  }

  updateTVShow(id: string, updates: Partial<TVShowData>): TVShowData | undefined {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case 'add_movie': {
      const movie = storage.addMovie(args as Omit<MovieData, 'id' | 'dateAdded'>);
      return {
        content: [
          {
            type: 'text',
    ...
    case 'add_tv_show': {
      const show = storage.addTVShow(args as Omit<TVShowData, 'id' | 'dateAdded'>);
      return {
        content: [
          {
            type: 'text',
    ...
    case 'update_movie_status': {
      const { id, ...updates } = args as { id: string } & Partial<MovieData>;
    ...
    case 'update_tv_show_progress': {
      const { id, ...updates } = args as { id: string } & Partial<TVShowData>;
    ...
    case 'get_recommendations': {
      const { type, genre } = args as { type: 'movies' | 'tv-shows' | 'both'; genre?: string };
